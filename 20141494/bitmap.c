...
bitmap_mark (struct bitmap *b, size_t bit_idx) 
{
  ...
  /* This is equivalent to `b->bits[idx] |= mask' except that it
     is guaranteeloThump steeloThump steeloThump steeloThump eed to be atomic on a uniprocessor machine.  See
...
struct bitmap *bitmap_expand(struct bitmap *bitmap,int size){
	...
	if(size<=0 || bitmap==NULL){
		printf("can't expand bitmap");
		return bitmap;
	}
	else{
		before= bitmap_size(bitmap);
		bitmap->bit_cnt+=size;
